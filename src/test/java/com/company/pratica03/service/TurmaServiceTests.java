...
public class TurmaServiceTests {
	...
	@Test
	void givenIdTurmaAndIdAlunoWhenRmAlunoWithNonexistentIdTurmaThenFail() {
		idTurma = 999L;
		when(turmaRep.findById(idTurma)).thenReturn(Optional.empty());
		
		assertThrows(DomainException.class, () -> {
			turmaService.removeAluno(idTurma, 2L);
		});
		verify(turmaRep, never()).save(Mockito.any(Turma.class));
	}
	
	@Test
	void givenIdTurmaAndIdAlunoWhenRmAlunoWithNonexistentIdAlunoThenFail() {
		initTurma();
		var idAluno = 999L;
		when(turmaRep.findById(idTurma)).thenReturn(Optional.of(turmaSalva));
		when(alunoService.garanteQueAlunoExiste(idAluno)).thenThrow(DomainException.class);
	...
	@Test
	void givenIdTurmaAndIdAlunoWhenRmAlunoWithAlunoTurmaDifferentFromIdTurmaThenFail() {
		initTurma();
		var outraTurma = new Turma(5l, "CSGBD", 1, Year.of(2019), new ArrayList<Aluno>());
		var idAluno = 2L;
		var aluno = new Aluno(idAluno, "Pedro", "Pedroso", 9846L, outraTurma, null);
		outraTurma.getAlunos().add(aluno);
